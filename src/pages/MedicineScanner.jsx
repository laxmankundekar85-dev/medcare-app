...
export default function MedicineScanner({ onBack }) {
  ...
  return (
    <div className="max-w-md mx-auto space-y-6 pb-28 font-sans px-1">
      {/* Hidden File Input */}
      <input
        type="file"
        accept="image/*"
      ...
      <div className="flex items-center justify-between py-2">
        <button 
          ...
          className="p-2 -ml-2 text-slate-700 hover:text-teal-800 transition cursor-pointer"
        >
          <ArrowLeft size={22} />
        </button>
        <h1 className="text-xl font-bold text-teal-800 flex-1 text-center pr-6">
          Medicine Scanner
        </h1>
      </div>
      ...
      <div className="flex flex-col items-center text-center space-y-4 pt-2">
        <div className="relative flex items-center justify-center w-36 h-36 rounded-full bg-teal-50/80 border border-teal-200/60 shadow-inner">
          <div className="w-28 h-28 rounded-full bg-teal-100/60 flex items-center justify-center border border-dashed border-teal-300">
            <div className="w-16 h-16 rounded-2xl bg-teal-800 text-white flex items-center justify-center shadow-md">
              <svg 
                className="w-10 h-10" 
                viewBox="0 0 24 24" 
                fill="none" 
      ...
      <div className="space-y-3 pt-2">
        ...
        <button
          ...
          className="w-full bg-slate-50 hover:bg-teal-50/50 border border-slate-200/80 rounded-2xl p-4 flex items-center justify-between transition group shadow-sm text-left cursor-pointer"
        >
      ...
      {imagePreview && (
        <div className="bg-white border border-teal-200 rounded-2xl p-4 space-y-4 shadow-sm animate-in fade-in duration-300">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <span className="text-xs font-bold text-slate-700">Selected Medicine Image</span>
            <button
              type="button"
              onClick={handleReset}
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-3.5 bg-teal-800 hover:bg-teal-900 text-white font-bold rounded-xl transition shadow-md disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2 text-sm"
          >
            {loading ? (
              <>
      ...
      {scanResult && (
        <div className="bg-white border border-teal-200 rounded-2xl p-5 shadow-sm space-y-3.5">
          <div className="flex items-center gap-2 text-teal-800 font-bold text-sm border-b border-slate-100 pb-2.5">
            <CheckCircle2 size={20} className="text-teal-600" />
            Analysis Complete
          </div>

          <div className="space-y-2.5 text-xs text-slate-800 leading-relaxed whitespace-pre-line">
            {scanResult}
          </div>
      ...
      {/* How It Works Card */}
      <div className="bg-slate-50/80 border border-slate-200/70 rounded-2xl p-4 space-y-3.5 shadow-xs">
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-3">
            <div className="w-6 h-6 rounded-full bg-teal-100/80 text-teal-800 font-bold text-xs flex items-center justify-center shrink-0">
              1
            </div>
            <p className="text-xs text-slate-600 leading-snug pt-0.5">
              Scan or snap a clear photo of the medication.
            </p>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <p className="text-xs text-slate-600 leading-snug pt-0.5">
              Medcare AI analyzes the visual data instantly.
            </p>
          </div>
